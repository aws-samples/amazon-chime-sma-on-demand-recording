...
# Image download URLS
IMAGE_URL_BANNER = (
    "https://raw.githubusercontent.com/aws-samples/amazon-transcribe-output-word-document/main/images/banner.png"
)
IMAGE_URL_SMILE = (
    "https://raw.githubusercontent.com/aws-samples/amazon-transcribe-output-word-document/main/images/smile.png"
)
IMAGE_URL_FROWN = (
    "https://raw.githubusercontent.com/aws-samples/amazon-transcribe-output-word-document/main/images/frown.png"
)
IMAGE_URL_NEUTRAL = (
    "https://raw.githubusercontent.com/aws-samples/amazon-transcribe-output-word-document/main/images/neutral.png"
)

# Additional Constants
# After n seconds pause by one speaker, put next speech in new segment
...
def write(cli_arguments, speech_segments, job_status):
    ...
    if len(speech_segments) == 0:
        ...
        if cli_arguments.analyticsMode:
            build_call_loudness_charts(
                document,
                speech_segments,
                data["ConversationCharacteristics"]["Interruptions"],
                data["ConversationCharacteristics"]["NonTalkTime"],
                data["ConversationCharacteristics"]["TalkTime"],
                tempFiles,
            )
        ...
        write_transcribe_text(
            table,
            sentimentEnabled or cli_arguments.analyticsMode,
            cli_arguments.analyticsMode,
            speech_segments,
            keyed_categories,
        )
        document.add_paragraph()
    ...
    object_name = "output/" + cli_arguments.inputJob + ".docx"
    s3.upload_file(cli_arguments.outputFile, RECORDING_BUCKET, object_name)
    output_presigned_url = s3.generate_presigned_url(
        "get_object", Params={"Bucket": RECORDING_BUCKET, "Key": object_name}, ExpiresIn=604800
    )
    recording_presigned_url = s3.generate_presigned_url(
        "get_object", Params={"Bucket": RECORDING_BUCKET, "Key": cli_arguments.recording_key}, ExpiresIn=604800
    )

    sns.publish(
        TopicArn=SNS_TOPIC,
        Message="Download your transcript here: "
        + output_presigned_url
        + "  \nDownload your recording here: "
        + recording_presigned_url,
        Subject="Post Call Transcription",
    )
...
def build_single_loudness_chart(
    ...
    if len(quiet_time["Seconds"]) > 0:
        axes.bar(
            quiet_time["Seconds"], quiet_time["dB"], label="Non-talk time", color="lightcyan", width=BAR_CHART_WIDTH
        )
...
def write_comprehend_sentiment(document, speech_segments, temp_files):
    ...
    speaker0xnew = np.linspace(
        speaker0timestamps[0], speaker0timestamps[-1], int((speaker0timestamps[-1] - speaker0timestamps[0]) + 1.0)
    )
    ...
    speaker1xnew = np.linspace(
        speaker1timestamps[0], speaker1timestamps[-1], int((speaker1timestamps[-1] - speaker1timestamps[0]) + 1.0)
    )
...
def handler(event, context):
    """
    Entrypoint for the containerized Lambda.
    """

    print(event)
    cli_args = Namespace(
        inputJob=event["callAnalyticsJobName"],
        sentiment="off",
        confidence="off",
        outputFile=None,
        keep=False,
    ...
    if cli_args.inputJob is not None:
        ...
        cli_args.inputFile = "/tmp/" + cli_args.inputJob + "-asrOutput.json"
        print(cli_args.inputFile)
        print(download_url)
        key = download_url.split("/", 4)[4]
        ...
        try:
            ...
            s3.download_file(RECORDING_BUCKET, key, cli_args.inputFile)
        ...
        if cli_args.outputFile is None:
            cli_args.outputFile = "/tmp/" + cli_args.inputJob + ".docx"
